...
description: Wire a SOUTH KOREAN address / postcode (우편번호) lookup into a form — a signup or profile address, a checkout shipping address, a saved address book, a branch or store an admin registers, a "주소 찾기" button, a zonecode / zipcode / postcode field, or a 도로명·지번 pair. The answer is always the Kakao (Daum) Postcode widget (https://postcode.map.kakao.com/guide) — one CDN script, entirely client-side, NO API key and NO registration — so never ask the user for a Kakao key (the key-bearing dapi.kakao.com Maps SDK is a different product) and never build a proxy, a scraper or a home-made postcode table. Covers popup `open()` vs embedded `embed()` (popup-blocker and onresize traps), the oncomplete result fields and the two that bite (`data.postcode` has been EMPTY since 2020 — read `data.zonecode`; `roadAddress`/`jibunAddress` come back empty on a 1:N mapping — fall back to `autoRoadAddress`/`autoJibunAddress`), the 참고항목 composition, the mandatory detail-address (상세주소) input, and how to model the result in Weegloo (ShortText postcode / address / addressDetail, `localized: false`, member addresses as `createdBy :self` private Content written over ACMA). SOUTH KOREA ONLY — it searches the Korean government address database, so a form that will hold foreign addresses gets a plain free-text field instead and the user is told why. English only.
---
