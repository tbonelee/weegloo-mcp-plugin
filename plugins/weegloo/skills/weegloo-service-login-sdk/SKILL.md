...
1. **Google Cloud Console → create an OAuth 2.0 Client ID** (this is what produces the `clientId` /
   ...
   - Open the **Credentials** page → **`https://console.cloud.google.com/apis/credentials`** (create or
     select a Google Cloud project first).
...
2. **Weegloo Console → ServiceLogin:**
   - `clientId` / `clientSecret` from the Google OAuth client above. **Blocking user-only inputs — ask the user for them by walking through step 1 (link + filled-in redirect URI), and do not report sign-in as done without them (pitfall G).**
